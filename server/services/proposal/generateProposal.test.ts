...
function deps(overrides = {}) {
  return {
    ...
    getCaseStudy: vi.fn().mockResolvedValue(caseStudy),
...
describe('generateProposal', () => {
  it('uses persisted analysis and exactly the selected indexed case studies before uploading the PPTX artifact', async () => {
    const testDeps = deps()
    const proposal = await generateProposal({ rfpId: 'rfp-1', selectedCaseStudyIds: ['case-1'] }, testDeps as any)
    ...
    expect(proposal).toMatchObject({ id: 'proposal-1', rfpId: 'rfp-1', selectedCaseStudyIds: ['case-1'], pdfStatus: 'not_requested' })
  })

  it('rejects empty selections before creating a proposal', async () => {
    const testDeps = deps()
    await expect(generateProposal({ rfpId: 'rfp-1', selectedCaseStudyIds: [] }, testDeps as any)).rejects.toMatchObject({ statusCode: 400 })
  ...
  it('rejects requested PDF export before creating a proposal when the converter is unavailable', async () => {
    ...
    await expect(generateProposal({ rfpId: 'rfp-1', selectedCaseStudyIds: ['case-1'], includePdf: true }, testDeps as any)).rejects.toMatchObject({ statusCode: 422 })
    expect(testDeps.insertProposal).not.toHaveBeenCalled()
  })
