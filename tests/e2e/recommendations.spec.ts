...
test('selects a recommendation and generates a proposal from the review page', async ({ page }) => {
  ...
  await page.route('**/api/rfps/rfp-e2e/recommendations', route => route.fulfill({ json: {
    analysis,
    recommendations: [{
      id: 'rfp-e2e:capability:cloud migration', rfpId: 'rfp-e2e', requirement: 'Cloud migration', requirementType: 'capability',
      ...
      matchedSlideExcerpts: [{ caseStudyId: 'case-1', caseStudyTitle: 'Cloud migration', caseStudyClient: 'Reference Bank', caseStudyIndustry: 'Banking', slideIndex: 1, title: 'Results', excerpt: '99.99% uptime', similarity: 0.9 }],
      explanationSource: 'ai', selected: true,
    }],
  } }))
  await page.route('**/api/proposals/generate', route => route.fulfill({ json: {
    id: 'proposal-e2e', rfpId: 'rfp-e2e', title: 'Proposal for Example Bank', status: 'completed', selectedRequirementGroupIds: ['rfp-e2e:capability:cloud migration'], selectedCaseStudyIds: ['case-1'],
  ...
  await page.getByRole('button', { name: 'Generate Proposal (1 groups selected)' }).click({ noWaitAfter: true })
